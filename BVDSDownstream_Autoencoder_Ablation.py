...
########################### Downstream Code for Autoencoder ONLY #############################################
    ...
    def __init__(self, test_pig, hidden_size, learning_rate, weight_decay, l1_lambda, dropout, device_to_use, max_epochs, hidden_layer, num_layers):
        ...
        if self.num_layers == 2:
            self.bvds_regressor = nn.Sequential(
                nn.Linear(hidden_size, self.hidden_layer), 
                nn.ReLU(),
                nn.Dropout(dropout),
...
class MyDataModule(L.LightningDataModule):
    ...
    def __init__(self, data_directory, num_pigs, test_pig_num, training_mode, label_name,
        ...
        self.bvds_mode = training_mode   # bvds_mode = 'classification' or 'regression' unchanged from original
...
#############################################################################################################
#############################################################################################################
############## MAIN CODE below, ONLY for Autoencoder Upstream Models ########################################
#############################################################################################################
...
def objective():
    ...
    num_pigs = 6
    window_size = 30 #placeholder, will load from autoreg model
    batch_size = 128
    ...
    hidden_size = config.hidden_size #placeholder, will load from autoreg model
...
############################################################################################################
################# Section is for TRAINING a new downstream model with autoencoding #########################
    ...
    if mode == 'train':
        ...
        for test_pig_num in test_pig_nums:
            ...
            window_size = autoreg_model.window_size
            forecast_size = autoreg_model.forecast_size
            ...
            data_module = MyDataModule(data_dir, num_pigs, test_pig_num, training_mode, label_name,
                                ...
                                window_size=window_size, forecast_size=forecast_size)
...
if __name__ == "__main__":
    ...
    if perform_sweep:
        ...
        wandb.agent(sweep_id, function=objective, count=3)
    else:
